...
Manager.prototype.connectNext=function() {
  ...
  die.device.gatt.connect({minInterval:15,maxInterval:50,slaveLatency:0,window:500,interval:500}).then(function(gatt){
    ...
    if(busy){
      die.error="BLE busy; retrying";
      if(self.queue.indexOf(die)<0)self.queue.unshift(die);
      self.changed({kind:"status",message:die.error});
      setTimeout(function(){self.connectNext();},750);
    } else {
      die.error=message.toLowerCase().indexOf("cancel")>=0?"Connect timeout":"Connect failed";
