...
var manager,screen=0,dashPage=0,history=[],stats={},popup,group=[],groupTimer,status="Starting",dragged=false;
...
function chooseType(die){
  var menu={"":{"title":die.name.substr(-8)},"< Back":function(){E.showMenu();draw();},
    ...
    "Connect":function(){
      if(die.connected)return E.showMessage("Already ONLINE","GoDice");
      if(die.connecting)return E.showMessage("Connection in progress","GoDice");
      E.showMenu();status="Connecting "+die.name.substr(-4);manager.connect(die);draw();
    },
    "Refresh":function(){
      if(!die.connected)return E.showMessage("Die is OFFLINE\nWake it and scan.","GoDice");
      E.showMessage("Requesting...","GoDice");
      manager.refresh(die).then(function(){setTimeout(function(){chooseType(die);},700);})
        .catch(function(e){E.showMessage("Write failed\n"+e,"GoDice");});
    },
    "Identify":function(){
      if(!die.connected)return E.showMessage("Die is OFFLINE\nWake it and scan.","GoDice");
      E.showMessage("Sending LED pulse...","GoDice");
      manager.identify(die).then(function(){E.showMessage("LED command sent","GoDice");setTimeout(function(){chooseType(die);},800);})
        .catch(function(e){E.showMessage("Write failed\n"+e,"GoDice");});
    }};
