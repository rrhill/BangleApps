...
function addRoll(event){
  var d=event.die,now=Date.now(),previous=history.length?history[history.length-1]:null;
  /* Some firmware sends move/tilt stable immediately after stable. Keep the
   * visible result but avoid logging and buzzing twice for the same outcome. */
  if(previous&&previous.id===d.id&&previous.v===event.value&&now-previous.t<500){
    popup={die:d,value:event.value,until:now+1300};Bangle.setLCDPower(1);draw();return;
  }
  var row={t:now,id:d.id,name:d.name,type:P.NAMES[d.type],v:event.value};
...
function onEvent(event){
  ...
  if(event.kind==="rolling"){status="ROLLING "+event.die.name.substr(-4);Bangle.setLCDPower(1);}
  if(event.kind==="stable"||event.kind==="tilt"||event.kind==="move"){
    status=P.NAMES[event.die.type]+" rolled "+event.value;addRoll(event);
  } else draw();
}
...
function dashboard(){
  ...
  for(var i=0;i<3&&start+i<manager.dice.length;i++){
    ...
    g.setColor(d.rolling?"#ff0":g.theme.fg2).drawString(d.rolling?"ROLLING":d.connected?"ONLINE":d.connecting?"CONNECT":"OFFLINE",169,y+19);
